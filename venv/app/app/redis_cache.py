...
from flask_caching.backends.base import BaseCache as CachingBaseCache



class CustomerJsonSerializer:
    def dumps(self, value: _t.Any, protocol: int = 0):
        try:
            print("dumps to cache")
    ...
    def loads(self, value: _t.Optional[bytes]):
        if value is None:
            print("value is None")
            return None
        try:
...
class CustomerRedisCache(BaseCache):
    ...
    def get(self, key: str) -> _t.Any:
        print(f"get key {self.key_prefix + key}")
    ...
    def set(self, key: str, value: _t.Any, timeout: _t.Optional[int] = None) -> _t.Any:
        ...
        print(f"set key: {self.key_prefix + key}")
...
class RedisCache(CachingBaseCache, CustomerRedisCache):
