...
class IncomeAndExpenseControl:
    ...
    def insert_fake_data(self, update_task_status_function):
        ...
        total = self.__kwargs['number']
        for now_number in range(total):
            print(f"新增第{now_number}筆")
            update_task_status_function(now_number, total)
            self.__random_value(select_income_and_expense, select_jar_name)
            self.__insert()

    def __update_task_status(self, number, task, total):
        if number % 100 == 0:
            print("除100於0")
