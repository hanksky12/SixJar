...
export  class CurrentRow {
  getInfo(button, method, today) {
    ...
    if (method == "新增"){
        ...
        array = this.takeCurrentRowDataToArray(button, [])
    }
    this.id = array[0]
    this.income_and_expense = array[1]
    this.jar_name = array[2]
    this.date = array[3]
    this.money = array[4]
    this.remark = array[5]
  }

  takeCurrentRowDataToArray(button, array) {
      ...
      if (childNodes.nodeType === 1) {
          array.push(childNodes.textContent)
        }
      }
      return array
    }
